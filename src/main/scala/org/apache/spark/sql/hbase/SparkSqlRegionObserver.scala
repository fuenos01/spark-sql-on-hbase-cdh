...
abstract class BaseRegionScanner extends RegionScanner {
  ...
  def next(result: java.util.List[Cell], limit: Int): Boolean = next(result)
  ...
  def nextRaw(result: java.util.List[Cell], limit: Int) = next(result, limit)
}
  ...
  override def postScannerOpen(e: ObserverContext[RegionCoprocessorEnvironment],
    ...
    if (serializedPartitionIndex == null) {
      ...
      if (SparkEnv.get == null) {
        val sparkConf = new SparkConf(true).set("spark.driver.host", "127.0.0.1").set("spark" +
            ".driver.port", "0")
      ...
      val subPlanRDD: RDD[InternalRow] = HBaseSerializer.deserialize(serializedRDD)
      ...
      val internalAccumulators = Seq(
        ...
        new Accumulator(
          0L, AccumulatorParam.LongAccumulatorParam, Some(InternalAccumulator
              .PEAK_EXECUTION_MEMORY), internal = true)
      )
      ...
      new BaseRegionScanner() {
        ...
        override def getBatch: Int = ???

        override def next(result: java.util.List[Cell], scannerContext: ScannerContext):
        Boolean = ???
      }
    }
  }
